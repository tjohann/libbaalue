 ...
 *    (./udp_mgmt_reboot_client -s baalue_master)
 */
int main(int argc, char *argv[])
{
	 ...
	 * reboot device
	 */
	baa_info_msg("try to reboot %s", server_name);
	...
	baa_wrap_close(fds);
	baa_info_msg("will sleep some time -> until node back again");
	sleep(20);

	for (err = 0; err < 10; err++) {
		fds = baa_inet_dgram_client(server_name, mgmt_port);
		if (fds == -1)
			baa_info_msg("could not connect to %s, try again",
				     server_name);
		sleep(1);
	}
	if (fds == -1) {
		baa_error_msg("could not (re)-connect to device %s",
			      server_name);
		usage(EXIT_FAILURE);
	}
